...
public:
	...
	void on_render(SDL_Renderer* renderer)
	{
		...
		rect_dst.x = (pos_center.x - size_background.x / 2);
		rect_dst.y = (pos_center.y - size_background.y / 2);
		rect_dst.w = size_background.x, rect_dst.h = size_background.y;
		SDL_RenderTexture(renderer, tex_background, nullptr, &rect_dst);

		rect_dst.x = (pos_center.x - size_foreground.x / 2);
		rect_dst.y = (pos_center.y - size_foreground.y / 2);
		rect_dst.w = size_foreground.x, rect_dst.h = size_foreground.y;
		SDL_RenderTexture(renderer, tex_foreground, nullptr, &rect_dst);
	}
