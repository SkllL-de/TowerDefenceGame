...
public:
	...
	void on_update(SDL_Renderer* renderer)
	{
		...
		SDL_DestroySurface(suf_text_background);
		SDL_DestroySurface(suf_text_foreground);
	}
	...
	void on_render(SDL_Renderer* renderer)
	{
		...
		static SDL_Texture* tex_home_avatar = tex_pool.find(ResID::Tex_Home)->second;
		...
		SDL_RenderTexture(renderer, tex_home_avatar, nullptr, &rect_dst);
		...
		for (int i = 0; i < (int)HomeManager::instance()->get_current_hp_num(); i++)
		{
			...
			SDL_RenderTexture(renderer, tex_heart, nullptr, &rect_dst);
		}
		...
		rect_dst.w = width_text, rect_dst.h = height_text;
		SDL_RenderTexture(renderer, tex_text_foreground, nullptr, &rect_dst);

		rect_dst.x = position.x + (78 - 65) / 2;
		rect_dst.y = position.y + 78 + 5;
		rect_dst.w = 65, rect_dst.h = 65;
		SDL_RenderTexture(renderer, tex_player_avatar, nullptr, &rect_dst);

		rect_dst.x = position.x + 78 + 15;
		rect_dst.y += 10;
		roundedBoxRGBA(renderer, rect_dst.x, rect_dst.y, rect_dst.x + width_mp_bar, rect_dst.y + height_mp_bar, 4,
			color_mp_background.r, color_mp_background.g, color_mp_background.b, color_mp_background.a);
		...
		roundedBoxRGBA(renderer, rect_dst.x, rect_dst.y, rect_dst.x + rect_dst.w, rect_dst.y + rect_dst.h, 2,
			color_mp_foreground.r, color_mp_foreground.g, color_mp_foreground.b, color_mp_foreground.a);
...
private:
	...
	const SDL_Color color_mp_background = { 48, 40 ,51, 255 };
	const SDL_Color color_mp_foreground = { 144, 121, 173, 255 };
