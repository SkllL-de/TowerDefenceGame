...
public:
	...
	void place_tower(TowerType type, const SDL_Point& idx)
	{
		...
		AudioManager::instance()->PlayMusic(audio_pool.find(ResID::Sound_PlaceTower)->second);
	}
	...
	void upgrade_tower(TowerType type)
	{
		...
		AudioManager::instance()->PlayMusic(audio_pool.find(ResID::Sound_TowerLevelUp)->second);
	}
