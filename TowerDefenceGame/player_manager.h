...
public:
	...
	void on_update(double delta)
	{
		...
		if (!is_releasing_flash && !is_releasing_impact)
		{
			...
			const SDL_FRect& rect_map = ConfigManager::instance()->rect_tile_map;
...
private:
	void on_release_flash()
	{
		...
		AudioManager::instance()->PlayMusic(audio_pool.find(ResID::Sound_Flash)->second);
	}
	...
	void on_release_impact()
	{
		...
		AudioManager::instance()->PlayMusic(audio_pool.find(ResID::Sound_Impact)->second);
	}
